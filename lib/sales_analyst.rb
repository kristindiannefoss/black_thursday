...
class SalesAnalyst
  ...
  def standard_deviation(numerators, denominator)
    Math.sqrt(numerators.reduce(:+)/denominator).round(2)
  end

  def average_items_per_merchant_standard_deviation
    av_it = average_items_per_merchant

    numerators = @sales_engine.merchants.all.map do |merchant|
      ((merchant.items.count) - av_it)**2
    end
    denominator = @sales_engine.merchants.count
    standard_deviation(numerators, denominator)
  end

  def merchants_with_high_item_count
    minimum_count = average_items_per_merchant +
  ...
  def average_unit_price_standard_deviation
    ...
    numerators = @sales_engine.items.all.map do |item|
      ((item.unit_price) - av_price)**2
    end
    denominator = @sales_engine.items.count
    standard_deviation(numerators, denominator)
  end
  ...
  def top_merchants_by_invoice_count
    minimum_count = average_invoices_per_merchant +
  ...
  def bottom_merchants_by_invoice_count
    maximum_count = average_invoices_per_merchant -
                    (average_invoices_per_merchant_standard_deviation * 2)
  ...
  def average_invoices_per_day_standard_deviation
    ...
    numerators = invoices_by_day.map do |day, invoices|
      (invoices - av_in)**2
    end
    denominator = 6
    standard_deviation(numerators, denominator)
  ...
  def top_days_by_invoice_count
    minimum_count = average_invoices_per_day +
  ...
  def invoice_status(status)
    ((@sales_engine.invoices.count_by_status[status].to_f /
      @sales_engine.invoices.count) * 100).round(2)
  end
